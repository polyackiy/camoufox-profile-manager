...
> **Статус:** `v0.3.0`, рабочее состояние. 407 тестов: 22 запускают настоящий
> браузер и проверяют, что видит страница, ещё 2 выполняются только с
> `--no-network` — так браузерный набор доказывает, что не нуждается в сети;
> покрытие 90%. REST API
...
- **Прокси, проверяемые до того, как им довериться.** HTTP, HTTPS и SOCKS;
  ...
  которое страница измеряет двумя строками JavaScript. Ответ остаётся в списке
  профилей — адрес выхода, страна и задержка под строкой прокси, зелёным,
  жёлтым или красным, — а выделенные профили проверяются одной кнопкой.
